...
async function baixarDbDoGitHub() {
  ...
  try {
    ...
    if (r.status === 200 && r.data && r.data.content) {
      ...
      return { db: parsed, sha: r.data.sha };
    }
    console.log('ℹ️ Arquivo de DB não encontrado no GitHub — usando seed/local.');
    return null;
  } catch (e) {
...
async function enviarDbParaGitHub(db) {
  ...
  try {
    ...
    if (r.status === 200 || r.status === 201) {
      ...
      console.log('✅ Backup enviado pro GitHub.');
    } else {
      ...
      if (r.status === 409 || r.status === 422) {
        ...
        if (fresh && fresh.sha) {
          ...
          if (r2.status === 200 || r2.status === 201) {
            ...
            console.log('✅ Backup enviado pro GitHub (após retry).');
          } else {
            console.error('⚠️ Falha no retry do backup GitHub:', r2.status, JSON.stringify(r2.data).slice(0,200));
          }
        }
      } else {
        console.error('⚠️ Falha ao enviar backup GitHub:', r.status, JSON.stringify(r.data).slice(0,200));
      }
    }
  } catch (e) {
    console.error('⚠️ Erro ao enviar backup GitHub:', e.message);
  }
}
...
function agendarBackupGitHub() {
  ...
  _uploadTimer = setTimeout(async () => {
    _uploadTimer = null;
    if (_dbCache) {
      await enviarDbParaGitHub(_dbCache);
    }
  }, 4000); // espera 4s sem novas salvadas pra mandar
}
