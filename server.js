  ...
  try {
    ...
    if (r.status === 200 && r.data) {
      ...
      if (r.data.content) {
        ...
        console.log('✅ DB baixada do GitHub (backup recuperado com sucesso).');
        return { db: parsed, sha };
      }
      // Caso B: arquivo grande (>1MB) — GitHub não retorna content inline.
      // Precisamos buscar via Git Blobs API (r.data.git_url) ou download_url.
      console.log('ℹ️ DB grande (>1MB) detectada no GitHub — baixando via Git Blobs API...');
      // Opção B1: Git Blobs API (sempre retorna base64, sem limite de 1MB)
      if (r.data.git_url) {
        // git_url vem completo (https://api.github.com/...); extrair o path
      ...
      // Opção B2: fallback via raw.githubusercontent.com (sem auth, público se repo for público)
      if (r.data.download_url) {
        const raw = await _baixarRawUrl(r.data.download_url);
        if (raw) {
          const parsed = JSON.parse(raw);
          console.log('✅ DB grande baixada do GitHub via raw URL (' + Math.round(raw.length/1024) + 'KB).');
          return { db: parsed, sha };
        }
      }
    ...
    // Se 404, arquivo não existe ainda (primeiro deploy) — normal
    if (r.status === 404) {
      console.log('ℹ️ Arquivo de DB ainda não existe no GitHub — usando seed/local (primeiro uso).');
      return null;
    }
    // Se 401/403 (token inválido/sem permissão), tenta raw URL como último recurso (repo público)
    if (r.status === 401 || r.status === 403) {
      console.log('ℹ️ Token GitHub rejeitado (HTTP ' + r.status + ') — tentando raw URL (repo público)...');
      ...
      if (raw) {
        try {
          const parsed = JSON.parse(raw);
          console.log('✅ DB baixada do GitHub via raw URL (' + Math.round(raw.length/1024) + 'KB) — token não era necessário.');
          // Tenta pegar o sha separadamente (sem auth pode falhar, mas o download já funcionou)
          return { db: parsed, sha: null };
        } catch (e) {
          console.error('⚠️ Raw URL retornou conteúdo inválido:', e.message);
        }
      }
    }
...
async function _enviarViaGitDataApi(encoded) {
  try {
    ...
    const blobSha = rBlob.data.sha;

    // 2) Pegar o SHA do commit HEAD da branch
    ...
    // 5) Criar commit apontando para a nova tree, com pai = HEAD
    ...
    if (rUpdateRef.status === 200) {
      // Atualiza o sha cache para o novo blob sha (para futuras operações)
      _ghShaCache = null; // invalida cache do Contents API
      console.log('✅ Backup grande enviado pro GitHub (Git Data API, ' + Math.round(encoded.length/1024) + 'KB encoded).');
    } else {
      console.error('⚠️ Git Data: falha ao atualizar ref:', rUpdateRef.status, JSON.stringify(rUpdateRef.data).slice(0,200));
    }
  } catch (e) {
    console.error('⚠️ Erro ao enviar backup (Git Data API):', e.message);
  }
}
