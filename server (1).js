...
function migrateDemoDb(db) {
  ...
  if (db._ultimaModificacao === undefined) db._ultimaModificacao = 0;
  return db;
}
...
// Timeout de 30s: se o GitHub não responder, aborta a requisição para não
// travar o processo inteiro (especialmente no Render free tier).
const GH_REQUEST_TIMEOUT_MS = 30000;
function ghRequest(method, urlPath, bodyObj) {
  return new Promise((resolve, reject) => {
    ...
    req.on('error', reject);
    // Timeout: aborta a requisição se o GitHub demorar demais
    const timeoutHandle = setTimeout(() => {
      req.destroy(new Error('GitHub API timeout (' + GH_REQUEST_TIMEOUT_MS + 'ms)'));
...
let _uploadPendente = false; // guard: evita uploads concorrentes (causam 422 "not a fast forward")
...
async function enviarDbParaGitHub(db) {
  if (!GITHUB_BACKUP_ATIVO) return false;
  // Guard: se já tem um upload em andamento, não inicia outro (evita race condition
  // no HEAD do Git que causa o erro 422 "Update is not a fast forward").
  // O dado será enviado pelo próximo ciclo (debounce ou periódico).
  if (_uploadPendente) {
    console.log('⏳ Upload já em andamento — pulando este ciclo (evita conflito no HEAD).');
    return false;
  }
  _uploadPendente = true;
  try {
    ...
    if (Buffer.byteLength(conteudo) <= CONTENTS_API_LIMIT) {
      return await _enviarViaContentsApi(encoded);
    }

    // Se for grande (>1MB), usa Git Data API (blobs + trees + commits + refs)
    return await _enviarViaGitDataApi(encoded);
  } finally {
    _uploadPendente = false;
  }
}
  ...
  try {
    ...
    if (r.status === 200 || r.status === 201) {
      ...
      if (r.status === 409 || r.status === 422) {
        console.warn('⚠️ Contents API: conflito de SHA (' + r.status + ') — re-buscando SHA e tentando de novo...');
        ...
        if (fresh && fresh.sha) {
          ...
          if (r2.status === 200 || r2.status === 201) {
            ...
            // Tenta Git Data API como último recurso
            console.log('ℹ️ Tentando Git Data API como fallback...');
            return await _enviarViaGitDataApi(encoded);
          }
        } else {
          console.error('⚠️ Não foi possível obter SHA fresco do GitHub para retry.');
        }
      } else {
...
async function _enviarViaGitDataApi(encoded) {
  ...
  let blobSha = null; // blob é imutável — criamos só uma vez e reutilizamos nas retentativas
  for (let tentativa = 1; tentativa <= MAX_TENTATIVAS; tentativa++) {
    try {
      // 1) Criar blob com o conteúdo (só na primeira tentativa — o blob é imutável)
      if (!blobSha) {
      ...
      // 2) Pegar o SHA do commit HEAD da branch (RE-LÊ a cada tentativa!)
      ...
      // re-lê o HEAD e tenta de novo. Na última tentativa, usa force: true.
...
function agendarBackupGitHub() {
  ...
  if (_primeiraSalvada) {
    ...
    if (_dbCache) {
      _tentarUploadComRetry(_dbCache, 2).catch((e) => {
        console.error('⚠️ Primeira salvada: erro no upload:', e.message);
      });
    }
    return;
  }
  ...
  _uploadTimer = setTimeout(async () => {
    _uploadTimer = null;
    if (_dbCache) {
      await _tentarUploadComRetry(_dbCache, 2); // até 2 retentativas imediatas
    }
  }, 4000);
}

// Tenta o upload e, se falhar, re-tenta com backoff exponencial (1s, 3s).
// Isso garante que mesmo que o Git Data API falhe (ex: race condition no HEAD),
// o backup seja refeito rapidamente sem esperar o próximo ciclo periódico (30s).
async function _tentarUploadComRetry(db, maxRetries) {
  for (let i = 0; i <= maxRetries; i++) {
    try {
      const ok = await enviarDbParaGitHub(db);
      if (ok) {
        _ultimoBackupTs = Date.now();
        return;
      }
      if (i < maxRetries) {
        const espera = 1000 * Math.pow(2, i); // 1s, 2s, 4s...
        console.warn('⚠️ Upload falhou (tentativa ' + (i + 1) + '/' + (maxRetries + 1) + ') — tentando de novo em ' + espera + 'ms...');
        await new Promise(r => setTimeout(r, espera));
      }
    } catch (e) {
      console.error('⚠️ Upload erro (tentativa ' + (i + 1) + '):', e.message);
      if (i < maxRetries) {
  ...
  console.warn('⚠️ Todas as retentativas de upload falharam — backup periódico tentará novamente em até 30s.');
}
...
function iniciarBackupPeriodico() {
  ...
  _backupIntervalTimer = setInterval(async () => {
    ...
    try {
      const ok = await enviarDbParaGitHub(_dbCache);
      if (ok) {
        _ultimoBackupTs = Date.now();
      } else {
        // NÃO atualiza _ultimoBackupTs → próxima rodada do intervalo tenta de novo
        console.warn('⚠️ Backup periódico falhou — tentará novamente na próxima janela.');
      }
    } catch (e) {
      console.error('⚠️ Backup periódico erro:', e.message);
    }
  }, BACKUP_INTERVALO_MS);
}

// Backup de emergencia: captura SIGTERM/SIGINT (Render manda SIGTERM antes de dormir)
// Render dá ~10-30s antes de matar o processo — precisamos enviar o backup rápido.
async function backupEmergencia(sinal) {
  ...
  // Salva arquivo local PRIMEIRO (rápido, síncrono) — garante que o dado está no disco
  // mesmo se o upload pro GitHub não terminar a tempo.
  if (_dbCache) {
    try { fs.writeFileSync(DB_FILE, JSON.stringify(_dbCache, null, 2)); } catch (e) {}
  }
  // Tenta enviar pro GitHub com retry (1 tentativa extra se falhar)
  if (GITHUB_BACKUP_ATIVO && _dbCache) {
    try {
      await _tentarUploadComRetry(_dbCache, 1);
      console.log('Backup final processado!');
    } catch (e) {
      console.error('Erro no backup final:', e.message);
    }
  }
  process.exit(0);
}
...
async function loadDemoDb() {
  ...
  // Busca GitHub e arquivo local EM PARALELO para comparar qual é mais recente
  const [remoto, localDb] = await Promise.all([
    baixarDbDoGitHub(),
    _lerDbLocal(),
  ]);

  const remotoDb = remoto && remoto.db ? migrateDemoDb(remoto.db) : null;
  const localDbMigrated = localDb ? migrateDemoDb(localDb) : null;

  // Estratégia de merge: usa o DB com MAIS dados (usuários + posts).
  // Isso previne que um restart do Render (que baixa versão antiga do GitHub)
  // sobrescreva dados mais recentes que estão no arquivo local mas não subiram.
  if (remotoDb && localDbMigrated) {
    const scoreRemoto = _scoreDb(remotoDb);
    const scoreLocal = _scoreDb(localDbMigrated);
    const tsRemoto = remotoDb._ultimaModificacao || 0;
    const tsLocal = localDbMigrated._ultimaModificacao || 0;

    console.log(`📊 DB GitHub: ${remotoDb.users.length} usuários, ${remotoDb.posts.length} posts, ts=${tsRemoto}`);
    console.log(`📊 DB local:  ${localDbMigrated.users.length} usuários, ${localDbMigrated.posts.length} posts, ts=${tsLocal}`);

    if (scoreLocal > scoreRemoto || (scoreLocal === scoreRemoto && tsLocal > tsRemoto)) {
      // O local é mais completo ou mais recente — USA O LOCAL e faz upload pro GitHub
      console.log('✅ DB local é mais recente/completo — usando local e sincronizando com GitHub...');
      _ghShaCache = remoto.sha || null;
      _dbCache = localDbMigrated;
      try { fs.writeFileSync(DB_FILE, JSON.stringify(_dbCache, null, 2)); } catch (e) {}
      // Sobe o DB local pro GitHub (sincroniza)
      _tentarUploadComRetry(_dbCache, 2).catch(e => console.error('Erro ao sincronizar local->GitHub:', e.message));
      return _dbCache;
    } else {
      // O GitHub é mais completo ou mais recente — usa o do GitHub
      console.log('✅ DB do GitHub é mais recente/completo — usando GitHub...');
  ...
  // Só tem GitHub (sem arquivo local — primeiro deploy ou FS efêmero limpo)
  if (remotoDb) {
    _ghShaCache = remoto.sha;
    _dbCache = remotoDb;
    try { fs.writeFileSync(DB_FILE, JSON.stringify(_dbCache, null, 2)); } catch (e) {}
    console.log('✅ DB carregada do GitHub (sem arquivo local para comparar).');
    return _dbCache;
  }

  // Só tem arquivo local (GitHub indisponível ou 404)
  if (localDbMigrated) {
    _dbCache = localDbMigrated;
    console.log('✅ DB carregada do arquivo local (GitHub indisponível).');
    return _dbCache;
  }

  // Nenhum dos dois — primeiro uso: cria o seed
  console.log('🌱 Primeiro uso — criando banco de dados inicial (seed)...');
  _dbCache = migrateDemoDb(seedDemoDb());
  _dbCache._ultimaModificacao = Date.now();
  saveDemoDb(_dbCache);
  return _dbCache;
}

// Helper: lê o DB do arquivo local (retorna null se não existir ou inválido)
function _lerDbLocal() {
  try {
...
// Helper: calcula um "score" do DB (total de entidades) para comparar qual versão é mais completa
function _scoreDb(db) {
  return (db.users || []).length + (db.posts || []).length + (db.comentarios || []).length
    + (db.curtidas || []).length + (db.amizades || []).length + (db.mensagens || []).length
    + (db.grupos || []).length + (db.mensagensGrupo || []).length + (db.recados || []).length;
}

async function saveDemoDb(db) {
  // Atualiza timestamp de modificação para merge no boot
  db._ultimaModificacao = Date.now();
  _dbCache = db;
  try {
...
// Versão de saveDemoDb para operações CRÍTICAS (cadastro de usuário, bio, posts).
// Faz upload IMEDIATO ao GitHub (sem debounce) e ESPERA o upload completar
// antes de retornar — garante que o dado está seguro no GitHub mesmo se o
// Render reiniciar logo depois. O arquivo local também é salvo sincronamente.
async function saveDemoDbCritico(db) {
  db._ultimaModificacao = Date.now();
  _dbCache = db;
  try {
    fs.writeFileSync(DB_FILE, JSON.stringify(db, null, 2));
  ...
  // Upload IMEDIATO e SÍNCRONO (espera completar) — operação crítica
  if (GITHUB_BACKUP_ATIVO) {
    console.log('🚨 Salvamento crítico — upload imediato ao GitHub (aguardando)...');
    try {
      await _tentarUploadComRetry(db, 3);
      console.log('✅ Upload crítico concluído com sucesso!');
    } catch (e) {
      console.error('⚠️ Erro no upload crítico (dado salvo localmente):', e.message);
    }
  }
}
